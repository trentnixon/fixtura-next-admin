...
export async function fetchClubContactInfo(): Promise<FetchClubContactInfoResponse> {
  try {
    ...
    if (error instanceof AxiosError) {
      ...
      console.error("[Axios Error] Failed to fetch club contact info:", {
      ...
      throw new Error(
        error.response?.data?.message ||
          `Failed to fetch club contact info: ${
      ...
      console.error(
        "[Unexpected Error] Failed to fetch club contact info:",
        error
      );

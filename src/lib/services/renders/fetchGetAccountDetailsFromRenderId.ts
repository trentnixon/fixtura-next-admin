...
import { GetAccountFromRender } from "@/types/render";

// Define the return type of the API response
interface FetchRenderResponse {
  data: GetAccountFromRender;
}

export async function fetchGetAccountDetailsFromRenderId(
  renderId: string
): Promise<GetAccountFromRender> {
  try {
    ...
    const response = await axiosInstance.get<FetchRenderResponse>(
      `/render/getAccountDetailsFromRenderId/${renderId}`
    );

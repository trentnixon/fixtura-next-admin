...
Created: 2026-05-20
Updated: 2026-05-20
Related: Roadmap-Fixture-Detail, Handoff-trigger-result-single-scrape

---

## Overview

Expose CMS `POST /api/game-meta-data/trigger-result-single-scrape` from the fixture detail actions bar so admins can queue a PlayHQ result scrape (`scrape:result-single`) for the current game-meta-data record.

## What We Need to Do

Add types, server service, React Query mutation hook, confirm-dialog UI with URL guards, wire `fixtureId` into `FixtureActionsBar`, invalidate `single-fixture-detail` on success, and document in repo readMes.

## Completion Summary

Implemented `triggerResultSingleScrape`, `useTriggerResultSingleScrape`, and `TriggerResultSingleScrapeButton` on `/dashboard/fixtures/[id]` with client-side guards for game-centre URLs; successful triggers invalidate the fixture detail query so refreshed scorecards load after the async job.
...
Queued fixture result scrape integration (CMS trigger endpoint, mutation hook, actions-bar button with confirmation).

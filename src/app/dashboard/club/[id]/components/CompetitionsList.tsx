...
import { calculateCompetitionTimeline, cn } from "@/lib/utils";
...
function formatParticipationMethod(method: string): string {
  switch (method) {
...
function CompetitionRow({
  ...
  const externalUrl = competitionUrl ?? url;
    ...
    <TableRow>
      <TableCell>
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {association?.name ?? season ?? `Competition #${id}`}
          </p>
        </div>
      </TableCell>
      <TableCell>
      ...
      <TableCell>
        <Badge variant="outline" className="bg-slate-50 text-slate-600">
          {formatParticipationMethod(participationMethod)}
        </Badge>
      </TableCell>
